...
const ChatPanel = () => {
  ...
  const loadChats = async (opts?: { selectFirst?: boolean }) => {
    setLoadingChats(true); setError(null);
    try {
      const { data } = await chatService.listChats({ sortBy: "updatedAt", sortOrder: "desc", limit: 50 });
      setChats(data);
      if (opts?.selectFirst && data.length > 0) await handleSelectChat(data[0]._id);
    } catch { setError("Unable to load chat history."); } finally { setLoadingChats(false); }
  };

  const handleSelectChat = async (chatId: string) => {
    setActiveChat(chatId); setLoadingMessages(true); setError(null);
    try {
      const chat = await chatService.getChat(chatId);
      setMessages(chat.messages ?? []);
    } catch { setError("Unable to load this conversation."); setMessages([]); }
    finally { setLoadingMessages(false); }
  };

  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (modalRef.current && !modalRef.current.contains(e.target as Node)) setShowUserModal(false);
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  }, []);

  useEffect(() => { loadChats({ selectFirst: true }); }, []);
  ...
  const handleSend = async () => {
    ...
    try {
      if (!activeChat) {
        const newChat = await chatService.createChat({ title: content.slice(0, 60) || "New conversation", message: content });
        ...
        setChats((prev) => { const f = prev.filter((c) => c._id !== updated._id); return [updated, ...f]; });
      }
    } catch { setError("Unable to send message. Please try again."); }
  ...
  const handleNewChat = () => { setActiveChat(null); setMessages([]); };

  const handleDeleteChat = async (chatId: string) => {
    if (!window.confirm("Delete this chat?")) return;
    setDeletingId(chatId); setError(null);
    try {
      await chatService.deleteChat(chatId);
      setChats((prev) => prev.filter((c) => c._id !== chatId));
      if (activeChat === chatId) { setActiveChat(null); setMessages([]); }
    } catch { setError("Unable to delete chat."); } finally { setDeletingId(null); }
  };

  const handleAddProject = () => {
    if (!newProjectName.trim()) return;
    const colors = ["bg-emerald-500", "bg-pink-500", "bg-indigo-500", "bg-orange-500"];
    setProjects((p) => [...p, { id: Date.now().toString(), name: newProjectName.trim(), color: colors[p.length % colors.length] }]);
  ...
  const activeTitle = activeChat ? (chats.find(c => c._id === activeChat)?.title || "Conversation") : "New Conversation";
      ...
      <aside className="w-[260px] shrink-0 flex flex-col bg-sidebar border-r border-sidebar-border">
        ...
        <div className="flex items-center justify-between px-4 pt-4 pb-3">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-primary flex items-center justify-center shadow-[0_0_10px_1px_color-mix(in_srgb,var(--primary)_35%,transparent)]">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
            </div>
            <span className="text-[13px] font-bold tracking-tight">Kelo Chat</span>
          </div>
          <button
        ...
        <div className="px-3 pb-2">
          <div className="relative">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/40" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
            <input
              value={searchQuery}
        ...
        <div className="px-3 pt-2">
          <div className="flex items-center justify-between mb-1.5 px-1">
            <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground/50">Projects</p>
            <button onClick={() => setShowNewProject(v => !v)} className="w-5 h-5 flex items-center justify-center rounded text-muted-foreground hover:text-foreground hover:bg-sidebar-accent transition-colors">
          ...
          {showNewProject && (
            <div className="flex gap-1.5 mb-2">
              ...
              />
              <button onClick={handleAddProject} className="rounded-lg bg-primary px-2.5 py-1.5 text-xs font-bold text-primary-foreground hover:opacity-90 transition-opacity">Add</button>
            </div>
          )}

          <div className="space-y-0.5 mb-1">
            {projects.map((p) => (
              <button key={p.id} className="w-full flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-xs text-muted-foreground hover:bg-sidebar-accent hover:text-foreground transition-colors text-left">
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-2 space-y-0.5">
          <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground/50 px-1 mb-2">History</p>
          ...
          {!loadingChats && filteredChats.map((chat) => (
            <div
              ...
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); handleSelectChat(chat._id); } }}
            ...
            >
              <svg className="shrink-0 w-3.5 h-3.5 opacity-50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
              <div className="flex-1 min-w-0">
                <p className="truncate text-[11px] font-semibold">{chat.title}</p>
                <p className="text-[10px] text-muted-foreground/60">{new Date(chat.updatedAt).toLocaleDateString()}</p>
              </div>
              <button
              ...
              >
                {deletingId === chat._id
                  ...
                  : <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
                }
              </button>
            </div>
          ))}
          ...
          {!loadingChats && filteredChats.length === 0 && (
            <div className="flex flex-col items-center gap-2 py-8 text-center">
              <svg className="w-8 h-8 text-muted-foreground/25" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
        ...
        {/* User area */}
        <div className="relative border-t border-sidebar-border p-3" ref={modalRef}>
          {showUserModal && (
              ...
              <div className="p-1.5 space-y-0.5">
                <div className="flex items-center justify-between px-3 py-2 rounded-lg">
                  ...
                  <span className="flex items-center gap-1.5 text-xs font-semibold capitalize">
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />{user?.status}
                  </span>
                </div>
                <div className="flex items-center justify-between px-3 py-2 rounded-lg">
                  <span className="text-xs text-muted-foreground">Role</span>
                  <span className="text-xs font-semibold capitalize">{user?.role?.toLowerCase().replace("_", " ")}</span>
                </div>
              </div>
              <div className="p-1.5 border-t border-sidebar-border">
                <button onClick={() => { setShowUserModal(false); navigate({ to: "/dashboard" }); }}
          ...
          >
            ...
            <svg className="shrink-0 text-muted-foreground/40" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m18 15-6-6-6 6"/></svg>
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-background">
        ...
        <div className="flex items-center gap-3 px-6 py-3.5 border-b border-border/60 bg-background/80 backdrop-blur-xl shrink-0">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
          </div>
          <div className="flex-1">
            <p className="text-sm font-bold leading-tight">{activeTitle}</p>
            <p className="text-[11px] text-muted-foreground flex items-center gap-1.5 mt-0.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />Kelo AI · Online
            </p>
          </div>
          {activeChat && (
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
            ...
            <div className="space-y-4">
              {[50, 75, 60].map((w, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div className={`h-10 rounded-xl bg-muted animate-pulse`} style={{ width: `${w}%` }} />
                </div>
              ))}
            </div>
          )}
          ...
          {!loadingMessages && messages.length === 0 && !isSending && (
            <div className="h-full flex flex-col items-center justify-center text-center gap-4 py-16">
              <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center">
                <svg className="text-primary" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
              </div>
              <div>
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-2">
                {["What's the PTO policy?", "How do I request access?", "Explain the onboarding process"].map((s) => (
          ...
          {!loadingMessages && messages.map((msg, idx) => (
            <div key={msg._id || msg.timestamp || idx} className={`flex items-start gap-3 mb-5 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                ...
                {msg.metadata?.sources && msg.metadata.sources.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-current/15 flex flex-wrap gap-1">
                    {msg.metadata.sources.map((src, i) => (
                      <span key={i} className="text-[10px] font-semibold opacity-70 flex items-center gap-1">
                        📎 {src}
                      </span>
                    ))}
                  </div>
          ...
          {isSending && (
            <div className="flex items-start gap-3 mb-5">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[10px] font-bold bg-primary/10 text-primary">AI</div>
          ...
          <div className="relative flex items-end gap-3 rounded-2xl border border-border/70 bg-card px-4 py-3 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/40 transition-all shadow-sm">
            ...
            >
              {isSending
                ...
                : <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 2 11 13"/><path d="M22 2 15 22 11 13 2 9l20-7z"/></svg>
              }
            </button>
          </div>
