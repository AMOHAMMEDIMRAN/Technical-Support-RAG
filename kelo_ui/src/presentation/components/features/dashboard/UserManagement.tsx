...
import { userService, type CreateUserRequest } from "@/infrastructure/api/user.service";
...
import { UserPlus, RefreshCw, AlertTriangle, CheckCircle2, Users, Search, Trash2 } from "lucide-react";

const roleColors: Record<string, string> = {
  [UserRole.SUPER_ADMIN]: "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20",
  [UserRole.CEO]: "bg-primary/10 text-primary border-primary/20",
  [UserRole.MANAGER]: "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20",
  [UserRole.DEVELOPER]: "bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-500/20",
  [UserRole.SUPPORT]: "bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-500/20",
  [UserRole.HR]: "bg-pink-500/10 text-pink-600 dark:text-pink-400 border-pink-500/20",
  [UserRole.FINANCE]: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20",
};

const RolePill = ({ role }: { role: string }) => (
  <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide ${roleColors[role] ?? "bg-muted text-muted-foreground border-border"}`}>
    {role.replace("_", " ")}
  </span>
);
...
const UserManagement = () => {
  ...
  const [formData, setFormData] = useState<CreateUserRequest>({
    email: "", password: "", firstName: "", lastName: "", role: UserRole.SUPPORT,
  });
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setUsers(data.users || []); setError(null);
    } catch (err: any) { setError(err.response?.data?.error || "Failed to fetch users"); setUsers([]); }
    finally { setLoading(false); }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setCreateLoading(true); setCreateError(null); setCreateSuccess(false);
    try {
      await userService.createUser(formData);
      setCreateSuccess(true);
      setFormData({ email: "", password: "", firstName: "", lastName: "", role: UserRole.SUPPORT });
      await fetchUsers();
      setTimeout(() => { setDialogOpen(false); setCreateSuccess(false); }, 1500);
    } catch (err: any) { setCreateError(err.response?.data?.error || "Failed to create user."); }
  ...
  const handleDelete = async (userId: string) => {
    ...
    try { await userService.deleteUser(userId); await fetchUsers(); }
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-primary mb-1">Management</p>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="rounded-2xl border-border/70 shadow-2xl sm:max-w-md">
            ...
            <DialogHeader>
              ...
              <DialogDescription className="text-xs text-muted-foreground">Add a new member to your organization</DialogDescription>
            </DialogHeader>
            ...
            {createSuccess && (
              <div className="flex items-center gap-2.5 rounded-xl border border-emerald-400/30 bg-emerald-500/8 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-400">
                <CheckCircle2 className="w-4 h-4 shrink-0" /> User created successfully!
              </div>
            )}
            {createError && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4 mt-1">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <PremiumInput id="firstName" name="firstName" value={formData.firstName} onChange={handleChange} required disabled={createLoading} placeholder="John" />
                </div>
                <div className="space-y-1.5">
                  <FieldLabel>Last Name *</FieldLabel>
                  <PremiumInput id="lastName" name="lastName" value={formData.lastName} onChange={handleChange} required disabled={createLoading} placeholder="Doe" />
                </div>
              </div>
              <div className="space-y-1.5">
                <FieldLabel>Email *</FieldLabel>
                <PremiumInput id="email" name="email" type="email" value={formData.email} onChange={handleChange} required disabled={createLoading} placeholder="john@company.com" />
              </div>
              <div className="space-y-1.5">
                <FieldLabel>Password *</FieldLabel>
                <PremiumInput id="password" name="password" type="password" value={formData.password} onChange={handleChange} required disabled={createLoading} placeholder="Min. 6 characters" />
              </div>
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent className="rounded-xl border-border/70">
                    {Object.values(UserRole).filter(r => r !== UserRole.SUPER_ADMIN).map((role) => (
                      <SelectItem key={role} value={role} className="text-sm font-medium rounded-lg">
                        {role.replace("_", " ")}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <button type="submit" disabled={createLoading}
                ...
                {createLoading ? <><RefreshCw className="w-4 h-4 animate-spin" />Creating…</> : "Create User"}
              </button>
            </form>
          </DialogContent>
        </Dialog>
      ...
      <div className="rounded-2xl border border-border/70 bg-card overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-border/50">
          ...
          <div className="flex items-center gap-2 ml-auto text-xs text-muted-foreground">
            ...
            <span>{filtered.length} {filtered.length !== users.length ? `of ${users.length}` : ""} users</span>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border/50 bg-muted/20">
                  <th className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Member</th>
                  <th className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Email</th>
                  <th className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Role</th>
                  <th className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Status</th>
                  <th className="px-5 py-3" />
                </tr>
              </thead>
              <tbody>
                {filtered.map((user, i) => (
                  <tr key={user.id} className={`group border-b border-border/40 last:border-0 hover:bg-accent/30 transition-colors ${i % 2 !== 0 ? "bg-muted/10" : ""}`}>
                    ...
                    <td className="px-5 py-4">
                      <span className="flex items-center gap-1.5 text-xs font-semibold">
                        <span className={`h-1.5 w-1.5 rounded-full ${user.status === UserStatus.ACTIVE ? "bg-emerald-500 shadow-[0_0_5px_1px_rgba(16,185,129,0.5)]" : "bg-muted-foreground"}`} />
                        {user.status}
                      </span>
                    </td>
                    <td className="px-5 py-4">
